...
public abstract class TwoLineListAdapter<T> extends ArrayAdapter<T> {
	...
	public TwoLineListAdapter(Context context, int listItemResourceId, List<T> items) {
		this(context, listItemResourceId, -1, items);
	}

	public TwoLineListAdapter(Context context, int listItemResourceId, int iconDrawableId, List<T> items) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		// set bottom text view (the secondary text)
		tv = (TextView) view.findViewById(R.id.row1);
		if ( (mSecondRows != null) && (mSecondRows.get(position) != null) ) {
